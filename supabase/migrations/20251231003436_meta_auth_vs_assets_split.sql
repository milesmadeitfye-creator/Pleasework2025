/*
  # Meta Connection Status RPC - Auth vs Assets Split

  Updates the canonical Meta status RPC to distinguish between:
  1. Auth connection (OAuth + token valid)
  2. Assets configuration (required assets selected)

  This fixes the issue where users with valid OAuth tokens are blocked
  from Configure Assets because the RPC requires assets to return "connected".

  ## Changes

  1. New Fields
    - `auth_connected` - true if access_token exists and is valid
    - `assets_configured` - true only when required assets are selected
    - `missing_assets` - array of missing asset names

  2. Logic Split
    - Auth layer: Only checks access_token existence and validity
    - Assets layer: Checks ad_account_id, page_id, instagram_actor_id

  3. UI Impact
    - Configure Assets: Unlocked if auth_connected === true
    - Ad creation wizard: Unlocked if auth_connected === true
    - Final Publish: Requires assets_configured === true
*/

-- Drop existing function
DROP FUNCTION IF EXISTS public.get_meta_connection_status();

-- Create updated RPC with auth vs assets split
...
-- Add comment
COMMENT ON FUNCTION public.get_meta_connection_status() IS 'Returns Meta connection status with auth vs assets split. Auth = token valid, Assets = required assets configured. SECURITY DEFINER bypasses RLS.';
/*
  # Meta Connection Status - Canonical Source with Normalized IDs

  Updates get_meta_connection_status() to be the definitive canonical source
  of truth for Meta connection status. Adds normalized ad_account_id_raw for
  easier UI comparison.

  ## Changes

  1. Add ad_account_id_raw (numeric id without "act_" prefix)
  2. Return both ad_account_id (as stored) and ad_account_id_raw (normalized)
  3. Ensure this RPC is the ONLY source for connection/asset status
  4. All checkmarks in UI must use this RPC, not inferred from other tables

  ## Fields Returned

  - auth_connected: boolean (has valid access token)
  - assets_configured: boolean (has ad_account_id AND page_id)
  - ad_account_id: text (as stored, e.g., "act_123456789")
  - ad_account_id_raw: text (normalized, e.g., "123456789")
  - page_id: text
  - page_name: text
...
-- Create canonical RPC with normalized ad_account_id_raw
...
DECLARE
  ...
  -- Asset values (read from meta_credentials - CANONICAL SOURCE)
  v_ad_account_id text := null;
  v_ad_account_id_raw text := null;  -- Normalized (without "act_" prefix)
...
BEGIN
  ...
  -- Check Meta credentials - CANONICAL SOURCE OF TRUTH
  -- This is the ONLY table that should be checked for connection status
  SELECT
  ...
  -- UI can compare this raw ID with numeric values from Meta API
  ...
  -- Assets configured = auth connected AND has required assets
  v_assets_configured := (
  ...
  IF v_auth_connected AND v_ad_account_id IS NOT NULL AND (v_ad_account_name IS NULL OR v_ad_account_name = '') THEN
    ...
    WHERE user_id = v_user_id
      AND (ad_account_id = v_ad_account_id OR account_id = v_ad_account_id OR ad_account_id = v_ad_account_id_raw OR account_id = v_ad_account_id_raw)
    LIMIT 1;
  ...
  -- Build result with both layers + normalized raw ID
  v_result := jsonb_build_object(
    ...
    -- Asset details (canonical from meta_credentials)
    'ad_account_id', v_ad_account_id,
    'ad_account_id_raw', v_ad_account_id_raw,
...
-- Add comment
COMMENT ON FUNCTION public.get_meta_connection_status() IS 'CANONICAL source for Meta connection status. Returns auth_connected, assets_configured, and all asset IDs including normalized ad_account_id_raw. SECURITY DEFINER bypasses RLS. UI MUST use this RPC, not inferred from other tables.';
...
import MasterTour from './MasterTour';
...
export default function TourLauncher({ variant = 'button', onTourComplete }: TourLauncherProps) {
  const { user } = useAuth();
  ...
  const loadTourState = async () => {
    ...
    try {
      ...
      if (!data) {
        ...
        setTourState('paused');
        const completed = (data.completed_chapters?.length || 0) / 10;
    ...
    return (
      <>
        ...
        {tourState === 'paused' && (
          ...
          >
            <Play className="w-4 h-4" />
            Resume Tour ({Math.round(progress)}% complete)
          </button>
        )}
        ...
        {showTour && <MasterTour onComplete={handleTourComplete} onPause={handleTourPause} />}
      </>
    );
  }
      ...
      <>
        <div className="bg-gradient-to-br from-blue-900/20 to-blue-950/20 border border-blue-800/50 rounded-2xl p-6">
          ...
          <p className="text-sm text-gray-300 mb-4">
            {tourState === 'not-started' &&
              'A 10-chapter guided walkthrough that teaches you everything about running your music like a label.'}
